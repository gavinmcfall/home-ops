# Pelican Game Server Management: Panel + Wings on TrueNAS

Deploy Pelican Panel on Kubernetes and Wings daemon on TrueNAS for self-hosted game server management.

> [!NOTE]
> Pelican is the successor to Pterodactyl, offering the same functionality with active development and improved features.
...
- Deploy Pelican Panel on Kubernetes using GitOps
...
- Set up PocketID OAuth for SSO authentication
...
|-----------|----------|---------|
| Pelican Panel | Kubernetes | Web UI for managing game servers |
...
| PocketID | Kubernetes | OIDC provider for SSO (optional) |
...
- [ ] PocketID deployed (optional, for SSO)
...
1. **[Panel Deployment](./01-panel-deployment.md)** — Deploy Pelican Panel on Kubernetes
...
4. **[PocketID OAuth](./04-pocketid-oauth.md)** — Configure SSO with PocketID (optional)
...
| Resource | URL/Value |
|----------|-----------|
| Panel URL | `https://pelican.${SECRET_DOMAIN}` |
...
| PocketID URL | `https://id.${SECRET_DOMAIN}` |
...
- [OAuth not working](#oauth-troubleshooting) — Plugin installation or configuration issues
