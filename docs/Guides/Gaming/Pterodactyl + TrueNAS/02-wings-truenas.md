...
- [ ] Pterodactyl Panel running (from Part 1)
...
mkdir -p /mnt/storage0/game-servers/wings/{config,logs,tmp}
...
## Step 4: Create the Node in Pterodactyl

1. Go to **Pterodactyl Admin Panel** → **Nodes** → **Create New**
...
The config will look similar to:
...
api:
  ...
  ssl:
    enabled: true
    cert: /etc/letsencrypt/live/play.${SECRET_DOMAIN}/fullchain.pem
    key: /etc/letsencrypt/live/play.${SECRET_DOMAIN}/privkey.pem
  upload_limit: 100
system:
...
services:
  wings:
    ...
    volumes:
      ...
      - "./certs:/etc/letsencrypt/live/play.${SECRET_DOMAIN}"
```

> [!IMPORTANT]
> The `/tmp/pterodactyl` volume **must** use the same path on host and container (not `./tmp:/tmp/pterodactyl`). Wings creates install scripts in `/tmp/pterodactyl/` and spawns containers that bind-mount this path from the **host**. If the paths don't match, installs will fail with "bind source path does not exist".
...
sudo chown -R apps:apps /tmp/pterodactyl
```
...
## Step 9: Verify Node Connection

1. Go to **Pterodactyl Admin Panel** → **Nodes** → Your node
...
**Symptoms:**
```
ERROR: failed to run install process for server error=Error response from daemon: invalid mount config for type "bind": bind source path does not exist: /tmp/pterodactyl/xxx
```

**Cause:** The `./tmp:/tmp/pterodactyl` volume mount creates the path inside the Wings container, but when Wings spawns install containers, Docker looks for `/tmp/pterodactyl` on the **host** filesystem.

**Fix:** Update docker-compose.yml to use matching paths:
```yaml
volumes:
  - "/tmp/pterodactyl:/tmp/pterodactyl"  # Same path on host and container
```
...
```bash
dig pterodactyl.${SECRET_DOMAIN}
```

**Check outbound connectivity:**
```bash
curl -v https://pterodactyl.${SECRET_DOMAIN}/api/application/nodes
```
