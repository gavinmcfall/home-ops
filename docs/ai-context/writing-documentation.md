...
**Invariant**: Use placeholder keys in manifests, resolve via ExternalSecrets.

`${SECRET_DOMAIN}` in HelmRelease values, ExternalSecret pulls from 1Password.
Never commit actual secret values - placeholders document required keys.
```
...
**Invariant**
Apps deploy via the kustomization + HelmRelease + ExternalSecret triad.

**Core Components** (details in manifests):
- [Kustomization](kubernetes/apps/...) - Flux entry point
- [HelmRelease](kubernetes/apps/...) - Helm configuration
- [ExternalSecret](kubernetes/apps/...) - Secret population

**Unified Workflow**:
1. Create app folder structure
2. Define HelmRelease with placeholders
3. Configure ExternalSecret
4. Run task configure
5. Push and watch Flux reconcile
```
...
```markdown
GOOD:
**Critical**: Never edit generated files under kubernetes/apps/*
[explanation]
Remember: Always run task configure before pushing.

BAD:
[long explanation]
"Oh by the way, don't edit generated files."
```

**Progressive Disclosure** (overview -> detail -> comprehensive):
```markdown
GOOD:
CLAUDE.md: "Use task configure to render templates" (with link)
->
WORKFLOWS.md: Why, how, gotchas about task configure
->
Taskfile.yaml: Complete task definitions
