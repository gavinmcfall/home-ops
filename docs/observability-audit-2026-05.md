...
- **Per-node power telemetry — true measurement.** The MS-01 (Venus Series, board AHWSA) does not expose IPMI/BMC to Talos (`/dev/ipmi0` absent, `/sys/class/ipmi` empty), no RAPL series. Intel AMT/vPro IS available on this board (BIOS visit required to enable, see `docs/infrastructure-roadmap/amt-enablement.md`) but research confirms AMT on MS-01 firmware does not surface power-draw sensors. The added `node_power_est_*` queries are a CPU-weighted estimate from aggregate UPS power — directional only. **Real measurement is available now via the JetKVM DC Power Control extension**: each JetKVM publishes `jetkvm_dc_power_watts` natively at `http://<jetkvm-ip>/metrics` (no auth required), measured by INA219 sensor between the wall adapter and the MS-01's DC barrel jack. Pending follow-up: add a ServiceMonitor + Endpoints for the 3 JetKVMs to wire them into Prometheus + kromgo. If a node's JetKVM is unavailable, fall back to dividing the relevant UPS's reading by the node count on that UPS.
