...
# kubectl port-forward service/sonarr-foreign -n downloads 8989:80
# export PUSHOVER_TOKEN="";
# export PUSHOVER_USER_KEY="";
# export sonarr-foreign_eventtype=Download;
# ./pushover-notify.sh
...
#
if [[ "${sonarr-foreign_eventtype:-}" == "Test" ]]; then
    PUSHOVER_TITLE="Test Notification"
    PUSHOVER_MESSAGE="Howdy this is a test notification from ${sonarr-foreign_instancename:-Sonarr-foreign}"
fi

#
# Send notification on Download or Upgrade
#
if [[ "${sonarr-foreign_eventtype:-}" == "Download" ]]; then
    if [[ "${sonarr-foreign_isupgrade}" == "True" ]]; then pushover_title="Upgraded"; else pushover_title="Downloaded"; fi
    printf -v PUSHOVER_TITLE "Episode %s" "${pushover_title}"
    printf -v PUSHOVER_MESSAGE "<b>%s (S%02dE%02d)</b><small>\n%s</small><small>\n\n<b>Client:</b> %s</small><small>\n<b>Quality:</b> %s</small>" \
        "${sonarr-foreign_series_title}" \
        "${sonarr-foreign_episodefile_seasonnumber}" \
        "${sonarr-foreign_episodefile_episodenumbers}" \
        "${sonarr-foreign_episodefile_episodetitles}" \
        "${sonarr-foreign_download_client}" \
        "${sonarr-foreign_episodefile_quality}"
    printf -v PUSHOVER_URL "%s/series/%s" "${sonarr-foreign_applicationurl:-localhost}" "${sonarr-foreign_series_titleslug}"
    printf -v PUSHOVER_URL_TITLE "View series in %s" "${sonarr-foreign_instancename:-Sonarr-foreign}"
fi

#
# Send notification on Manual Interaction Required
#
if [[ "${sonarr-foreign_eventtype:-}" == "ManualInteractionRequired" ]]; then
    PUSHOVER_PRIORITY="1"
    printf -v PUSHOVER_TITLE "Episode requires manual interaction"
    printf -v PUSHOVER_MESSAGE "<b>%s</b><small>\n<b>Client:</b> %s</small>" \
        "${sonarr-foreign_series_title}" \
        "${sonarr-foreign_download_client}"
    printf -v PUSHOVER_URL "%s/activity/queue" "${sonarr-foreign_applicationurl:-localhost}"
    printf -v PUSHOVER_URL_TITLE "View queue in %s" "${sonarr-foreign_instancename:-Sonarr-foreign}"
fi
