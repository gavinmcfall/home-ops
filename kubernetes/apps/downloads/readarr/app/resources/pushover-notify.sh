...
# kubectl port-forward service/readarr -n downloads 8989:80
...
if [[ "${readarr_eventtype:-}" == "Download" ]]; then
    ...
    printf -v PUSHOVER_MESSAGE "<b>%s (S%02dE%02d)</b><small>\n%s</small><small>\n\n<b>Client:</b> %s</small><small>\n<b>Quality:</b> %s</small>" \
        "${readarr_series_title}" \
        "${readarr_episodefile_seasonnumber}" \
        "${readarr_episodefile_episodenumbers}" \
        "${readarr_episodefile_episodetitles}" \
        "${readarr_download_client}" \
        "${readarr_episodefile_quality}"
    printf -v PUSHOVER_URL "%s/series/%s" "${readarr_applicationurl:-localhost}" "${readarr_series_titleslug}"
    printf -v PUSHOVER_URL_TITLE "View series in %s" "${readarr_instancename:-readarr}"
fi
