...
| -------------- | --------- | ----------------------- | ---------------------- | -------------- |
| Pasta (`pasta.${SECRET_DOMAIN}`) | entertainment | 3600 s proxy read/send timeout; homepage annotations | `HTTPRoute` with `timeouts.request/backendRequest=3600s`; reuse homepage annotations | Still served by ingress-nginx; HTTPRoute rollback until DNS strategy is sorted |
