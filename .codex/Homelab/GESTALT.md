...
## 🎯 The Core Pattern: Taskfile → Flux → ExternalSecrets

### Capsule: GitOpsPipeline ⚙️ AlwaysOn
...
### Capsule: GitOpsAuthority 🔒
`task configure` → `task kubernetes:kubeconform` → `flux diff` → PR → Flux reconcile. Manual `kubectl apply` is forbidden; Flux will revert it and the doc loses integrity (`ARCHITECTURE.md`, `CONTRACTS.md`).

### Capsule: PlaceholderDiscipline 🕵️
...
### Capsule: StorageFirst 📦
Persistence must be declared via PVC/NFS definitions before deployment. Example: ROMM’s `persistence.media`, `existingClaim: romm-data`, and `advancedMounts` (`kubernetes/apps/games/romm/app/helmrelease.yaml:90-150`). Missing a mount leaves pods `Pending`.
...
## 🧪 When Things Break
- **Flux stuck in `Reconciling`** → Check `flux get helmrelease <name>` then inspect placeholder values. Usually means ExternalSecret hasn’t synced secrets yet (`WORKFLOWS.md`).
- **`task configure` fails** → Missing placeholder in `bootstrap/config.yaml` or `.taskfiles` template. Document the new requirement in `TEMPLATE_GUIDE.md`.
- **Pods pending** → Look for `existingClaim`/`nfs` definitions in HelmRelease; ensure the PVC exists and names match (ROMM reference).
...
|-------|:------:|:----------:|---------|
| Taskfile → Flux → ExternalSecrets pipeline | `Taskfile.yaml`, `.taskfiles/Kubernetes.yaml`, `kubernetes/apps/games/romm/app/helmrelease.yaml` | 🟢 | Taskfile renders, Flux HelmRelease consumes, env values reference ExternalSecrets. |
| Placeholder + storage invariants | `kubernetes/apps/games/romm/app/helmrelease.yaml:65-150` | 🟢 | Hosts, env vars, `persistence` sections all rely on placeholders + declared PVCs. |
| Flux wiring requirement | `kubernetes/apps/games/kustomization.yaml` | 🟢 | HelmRelease directories listed; missing entry → Flux skip. |
| Bootstrapping layers exist | `bootstrap/`, `talosconfig/`, `scripts/` | 🟢 | Provide Talos machine configs + scripts referenced in README + Taskfile. |
